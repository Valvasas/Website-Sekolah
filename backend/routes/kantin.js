...
router.get('/products', authenticate, (req, res) => {
    ...
    const params = [];
    if (search) {
        conds.push('(p.name LIKE ? OR p.description LIKE ? OR u.nama_lengkap LIKE ?)');
...
router.post('/products', authenticate, (req, res) => {
    ...
    try {
        ...
        db.prepare(`
            ...
            (id,seller_id,seller_nisn,name,description,price,stock,image_url,chat_contact,emoney_provider,emoney_account,status,created_at,updated_at)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,'active',?,?)
        `).run(
            id,
            req.user.sub,
